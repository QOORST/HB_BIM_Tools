...
namespace YD_RevitTools.LicenseManager.Commands.MEP
{
    ...
    public sealed class CmdParameterCopy : IExternalCommand
    {
        ...
        private static bool Length(Parameter p) => p.StorageType == StorageType.Double && p.Definition.GetDataType() == SpecTypeId.Length;
        private static string Key(Parameter p) => p.IsShared ? "guid:" + p.GUID :
            p.Id.ToString().StartsWith("-", StringComparison.Ordinal) ? "builtin:" + p.Id : "name:" + p.Definition.Name;
        private static Parameter Resolve(Element e, string key)
        {
            var matches = e.Parameters.Cast<Parameter>().Where(p => Length(p) && Key(p) == key).ToList();
        ...
        private static Row Preview(Element e, Choice source, Choice target)
        {
            ...
            if (!(e is FamilyInstance) && !(e is MEPCurve)) row.Status = "不支援的物件";
            ...
            {
                row.Value = row.Source.AsDouble();
                row.Status = row.Target.HasValue && Math.Abs(row.Target.AsDouble() - row.Value) < Tolerance ? "無變更" : "可複製";
            }
            return row;
        }
        ...
        private static void Apply(Document doc, Row row)
        {
            ...
            using (var tx = new Transaction(doc, "參數複製 " + row.Element.Id))
            {
                ...
                try
                {
                    ...
                    if (!row.Target.Set(row.Value)) throw new InvalidOperationException("參數寫入失敗");
                    doc.Regenerate();
                    if (Math.Abs(row.Target.AsDouble() - row.Value) > Tolerance || !network.Select(Capture).Zip(original, Same).All(v => v))
                    ...
                    if (!network.Select(Capture).Zip(original, Same).All(v => v))
        ...
        public Result Execute(ExternalCommandData data, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                var choices = selected.SelectMany(e => e.Parameters.Cast<Parameter>()).Where(Length)
                ...
                if (choices.Count < 2) throw new InvalidOperationException("選取範圍沒有足夠的實例長度參數。");
                Show(ui.Document, selected, choices);
        ...
        private static void Show(Document doc, List<Element> selected, List<Choice> choices)
        {
                ...
                StartPosition = F.FormStartPosition.CenterScreen })
            {
                ...
                layout.RowStyles.Add(new F.RowStyle(F.SizeType.AutoSize));
                layout.RowStyles.Add(new F.RowStyle(F.SizeType.AutoSize));
                layout.Controls.Add(new F.Label { Text = "參數複製  |  已選取 " + selected.Count + " 個元素", AutoSize = true, Padding = new F.Padding(0, 0, 0, 12) });
                ...
                var grid = new F.DataGridView { Dock = F.DockStyle.Fill, ReadOnly = true, AllowUserToAddRows = false,
                ...
                Action render = () =>
                {
                    ...
                    foreach (var r in rows)
                        grid.Rows.Add(r.Element.Id.ToString(), r.Element.Name, r.Target?.AsValueString() ?? "", r.Source?.AsValueString() ?? "", r.Status);
                    summary.Text = "可複製 " + rows.Count(r => r.Status == "可複製") + " 個；無變更 " + rows.Count(r => r.Status == "無變更") +
                        " 個；已複製 " + rows.Count(r => r.Status == "已複製") + " 個；其餘 " + rows.Count(r => r.Status != "可複製" && r.Status != "無變更" && r.Status != "已複製") + " 個";
                    apply.Enabled = rows.Any(r => r.Status == "可複製");
                };
                Action preview = () => { if (source.SelectedItem == null || target.SelectedItem == null) return;
                ...
                source.SelectedItem = choices.FirstOrDefault(c => c.Key == sourceKey) ?? choices.FirstOrDefault(c => c.Name == "距離樓層的高程") ?? choices[0];
                target.SelectedItem = choices.FirstOrDefault(c => c.Key == targetKey) ?? choices.FirstOrDefault(c => c.Name == "立面高程") ?? choices[1];
                apply.Click += (s, e) =>
                {
                    try
                    {
                    preview();
                    var candidates = rows.Where(r => r.Status == "可複製").ToList();
