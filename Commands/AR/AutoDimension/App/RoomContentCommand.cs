    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            if (result.Changed == 0)
            {
                ...
                TaskDialog.Show("YD BIM Auto Dimension", reason);
                return Result.Cancelled;
            }

            tx.Commit();
            TaskDialog.Show("YD BIM Auto Dimension", $"已更新 {result.Changed} / {result.Processed} 個房間內容。");
        ...
        {
            message = ex.Message;
            TaskDialog.Show("YD BIM Auto Dimension", $"房間內容更新失敗：{ex.Message}");
            return Result.Failed;
        }
    }
}
