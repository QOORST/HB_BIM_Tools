    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        try
        {
            ...
            if (result.Changed == 0)
            {
                ...
                TaskDialog.Show("HB_BIM 房間內容", reason);
                return Result.Cancelled;
            }

            tx.Commit();
            TaskDialog.Show("HB_BIM 房間內容", $"已更新 {result.Changed} / {result.Processed} 個房間內容。");
        ...
        {
            message = ex.Message;
            TaskDialog.Show("HB_BIM 房間內容", $"房間內容更新失敗：{ex.Message}");
            return Result.Failed;
        }
    }
}
