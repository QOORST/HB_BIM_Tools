...
### 安全性改進
- 更新原生 SQLite 相依套件，排除 `GHSA-2m69-gcr7-jv3q` 高風險弱點。
...
### 改進
- 強化授權機器碼指紋，改以 CPU ID、BaseBoard Serial 與 MachineName 組合產生，同時保留既有授權相容性。
- 更新啟動流程移除暫存命令檔，下載後直接等待 Revit 關閉再啟動安裝器，降低更新流程被替換的風險。
- 授權驗證改用 `RSA.Create()` 匯入公開金鑰參數，取代舊式 `RSACryptoServiceProvider` API。
- 安裝檔 payload 準備流程改由版本清單集中管理，降低新增或調整 Revit 年版時漏改的風險。
- 安裝包只包含執行期資源，排除圖示產生腳本與規劃文件等開發用檔案。

### 整理
- 移除舊備份程式碼、一次性修復腳本、舊版文件封存與手動部署腳本。
- 正式部署流程統一為 `Installer\Build_Installer.ps1`。
