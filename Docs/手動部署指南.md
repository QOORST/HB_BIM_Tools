...
2. **複製主程式**：
   ...
   - 到 `C:\ProgramData\Autodesk\Revit\Addins\2024\HB_BIM\`
...
4. **複製圖示資源**：
   - 從 `Resources\Icons\`
   - 到 `C:\ProgramData\Autodesk\Revit\Addins\2024\HB_BIM\Resources\Icons\`

5. **創建 .addin 檔案**：
   - 在 `C:\ProgramData\Autodesk\Revit\Addins\2024\` 創建檔案
   - 檔名：`HB_BIM_Tools.addin`
   - 內容見下方

### Revit 2025 / 2026

步驟相同，只需將路徑中的 `2024` 替換為 `2025` 或 `2026`，並使用對應版本的 DLL。`Resources` 資料夾也需一併複製，否則 Pipe Sleeve 在沒有公司網路族庫時無法自動載入預設套管族群。

## .addin 檔案內容

創建文字檔案 `HB_BIM_Tools.addin`，內容如下：
...
釋疑簡報工具的歷史追蹤使用 SQLite，手動部署時必須一併複製上述 SQLite DLL 與 `runtimes` 資料夾，否則歷史紀錄、進度追蹤與 Excel 匯出可能無法啟動。
