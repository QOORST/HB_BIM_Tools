...
2. 使用 Inno Setup Compiler 編譯 `HB_BIM_Setup.iss`
...
簡要步驟：
1. 創建目標資料夾：`C:\ProgramData\Autodesk\Revit\Addins\2024\HB_BIM\`
2. 複製所有 DLL 檔案
...
注意：釋疑簡報工具會使用 SQLite 追蹤資料庫。部署包需包含 `Microsoft.Data.Sqlite.dll`、`SQLitePCLRaw.*.dll` 與 `runtimes\win-x64\native\e_sqlite3.dll`；自動部署與 Inno Setup 安裝檔會自動包含這些檔案，手動複製時請一併帶入。
