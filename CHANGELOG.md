...
- 保留正常樓梯踢面，以宿主實際實體裁除立板侵入部分，不採整批排除踢面。
- 納入同一宿主內相連共平面側模合併，以及牆梯雙向交界扣除。
- 累積納入梁板交界、非結構牆排除扣面、匯出與刪除識別、分析範圍、取消及設定記憶調整。
- 牆梯扣除已有使用者模型確認；立板裁切仍待模型回歸，離線測試不代表 Revit 原生幾何驗證。
- 支援安裝版本為 2022、2024、2025、2026；操作步驟及限制見 README.txt。

### 修正

- 修復公司族庫按鈕在其他電腦出現「找不到族庫資料庫」錯誤的問題。
- 族群資料庫按鈕現在同時檢查 DLL 和資料庫檔案是否存在，避免 DLL 存在但資料庫缺失時仍可點擊導致錯誤。
- 安裝腳本自動部署 `family_library.sqlite` 資料庫（如果存在），確保所有公司電腦族庫功能完整。
...
- 更新安裝改由獨立背景程序等待 Revit 結束，避免宿主關閉時等待工作一併停止。
- 背景程序啟動前確認就緒，安裝前再次比對 SHA-256，並記錄啟動結果。
- 舊版本若未啟動安裝，需關閉 Revit 後手動執行新版安裝檔一次。

## [2.5.13] - 2026-09-08

### 改進

- 更新內建 `套管-圓形_無.rfa` 族群至 `2026.09.08.01`，既有模型載入舊版族群時會自動覆蓋更新。
- 自動標註設定視窗加入高 DPI、可用螢幕範圍與窄視窗響應式配置，改善低解析度及多螢幕環境的文字、欄位與操作按鈕裁切問題。
- 自動標註介面統一繁中文字型、頁籤、按鈕與狀態回饋；設定資料改存於 `HB_BIM_Tools`，並自動沿用舊 `YD_BIM_Tools` 設定。

### 安全性改進

- 發版腳本只接受與安裝器公開憑證指紋相符的私鑰；找不到相符私鑰時中止建置，避免發布未簽章檔案。沿用既有 LAN 簽章憑證。
...
- 覆寫 `Microsoft.Data.Sqlite` 間接帶入的原生 SQLite 套件，改用 `SQLitePCLRaw.lib.e_sqlite3 3.53.3`，排除 `GHSA-2m69-gcr7-jv3q` 高風險弱點。
- 解除安裝 HB_BIM Tools 時，同步移除安裝程式加入的 LAN 自簽憑證，避免信任項目長期殘留。

### 注意事項

- AI 助理維持本地端／選配功能，本版不包含 AI 服務上線調整。
