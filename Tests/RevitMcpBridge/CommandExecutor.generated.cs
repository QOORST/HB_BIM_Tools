...
namespace RevitMCP.Core
{
    ...
    public class CommandExecutor
    {
        ...
        public RevitCommandResponse ExecuteCommand(RevitCommandRequest request)
        {
            try
            {
                ...
                switch (request.CommandName.ToLower())
                {
                    ...
                    case "hb_diagnose_grid_dimensions":
                        var assembly = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name == "YD_RevitTools.LicenseManager");
                        var method = assembly?.GetType("YDBIM.AutoDimension.Core.AutoDimensionDiagnostics")?.GetMethod("DiagnoseGrids");
                        if (method == null) throw new InvalidOperationException("請載入含診斷入口的 HB_BIM 版本。");
                        try { result = JObject.Parse((string)method.Invoke(null, new object[] { _uiApp.ActiveUIDocument.Document })); }
                    ...
                    case "create_ceiling":
                        result = CreateCeiling(parameters);
                        break;
        ...
        /// 建立天花板（根據房間邊界）
        /// </summary>
        private object CreateCeiling(JObject parameters)
        {
            Document doc = _uiApp.ActiveUIDocument.Document;
            int roomId = parameters["roomId"]?.Value<int>() ?? 0;
            double offsetHeight = parameters["offsetHeight"]?.Value<double>() ?? 3000; // 預設 3000mm (FL+300cm)

            if (roomId == 0)
            {
                throw new Exception("必須提供房間 ID");
            }
            ...
            if (room.Area <= 0)
            {
                throw new Exception($"房間 {room.Number} 沒有封閉邊界，無法建立天花板");
            }

            using (Transaction trans = new Transaction(doc, "建立天花板"))
            {
                trans.Start();
                ...
                if (level == null)
                {
                    throw new Exception("找不到房間所在的樓層");
                }

                // 計算天花板高度（樓層標高 + 偏移）
                double ceilingHeight = level.Elevation + (offsetHeight / 304.8); // 轉換為英尺
                ...
                if (ceilingType == null)
                {
                    throw new Exception("找不到可用的天花板類型");
                }
                ...
                IList<IList<BoundarySegment>> boundarySegmentsList = room.GetBoundarySegments(boundaryOptions);

                if (boundarySegmentsList == null || boundarySegmentsList.Count == 0)
                {
                    throw new Exception($"無法取得房間 {room.Number} 的邊界");
                }

                // 建立天花板邊界曲線列表
                List<Curve> curves = new List<Curve>();
                foreach (BoundarySegment segment in boundarySegmentsList[0])
                {
                ...
                // 建立天花板（使用 Ceiling.Create 靜態方法）
                Ceiling ceiling = Ceiling.Create(doc, curveLoops, ceilingType.Id, level.Id);

                if (ceiling == null)
                {
                    throw new Exception("建立天花板失敗");
                }
                ...
                {
                    heightOffsetParam.Set(offsetHeight / 304.8); // 轉換為英尺
                }
                ...
                {
                    Success = true,
                    CeilingId = ceiling.Id.IntegerValue,
                    RoomId = roomId,
                    RoomNumber = room.Number,
                    RoomName = room.get_Parameter(BuiltInParameter.ROOM_NAME)?.AsString(),
                    Level = level.Name,
                    OffsetHeight = offsetHeight,
                    Message = $"成功為房間 {room.Number} 建立天花板，高度: FL+{offsetHeight}mm"
                };
            }
        }
